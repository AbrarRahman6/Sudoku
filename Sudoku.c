...
int solve_sudoku(Cell mat[N][N], int row, int col) {
    if (row < N && col < N) {
        if (mat[row][col].value != 0) {
            if ((col + 1) < N) return solve_sudoku(mat, row, col + 1);
            else if ((row + 1) < N) return solve_sudoku(mat, row + 1, 0);
            else return 1;
        } else {
            for (int num = 1; num <= N; num++) {
                if ((CheckIfSafe(row, col, num))) {
                    ...
                    if (solve_sudoku(mat, row, col)) return 1;
                    else mat[row][col].value = 0;
                }
            }
        }
